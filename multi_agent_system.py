...
9. **Recovery Concordance Assessment (CRITICAL):** The strongest predictor of ALF outcome is whether improvement is CONCORDANT or DISCORDANT across organ systems:
   ...
   - **EARLY CRITICAL METABOLIC WARNING (Day 1-3, BINDING):** At Day 1-3, patients may show PARTIAL improvement (e.g., INR declining, creatinine improving) while retaining critical metabolic derangements that predict death. This check evaluates CURRENT VALUES at the time of assessment, NOT trends. When ANY of the following are present at Day 1-3, predict DEATH regardless of other improvements: (a) ammonia >150 umol/L -- this is BINDING: ammonia >150 carries significant ICH risk (Kumar/Bernal) REGARDLESS of whether it is declining from a higher value. An ammonia of 152 declining from 487 is STILL >150 and STILL carries ICH risk. The absolute value determines danger, not the direction. Do NOT rationalize that a declining ammonia is "improving" when it remains above the critical threshold. (b) lactate >4 mmol/L AND not declining from prior days (stagnant or rising), (c) bilirubin actively rising (upward TREND through Days 1-3). The combination of ammonia >150 + stagnant/rising lactate + rising bilirubin at Day 3 is particularly ominous: it indicates ongoing hepatic necrosis despite partial coagulation improvement. INR improvement alone at Day 3 does NOT establish recovery -- the full metabolic picture must be improving. This is BINDING and FINAL: do NOT override this warning with partial improvement in INR or creatinine. This is distinct from Day 5+ demonstrated recovery where multi-day concordant improvement IS conclusive.
...
- **MANDATORY PRE-CHECK B -- EARLY CRITICAL METABOLIC WARNING (Day 1-3, BINDING):** At Day 1-3, check CURRENT VALUES (not trends): ammonia >150 umol/L AND/OR lactate >4 not declining AND/OR bilirubin actively rising = predict DEATH. This is BINDING and FINAL. CRITICAL: ammonia >150 means the CURRENT reading is above 150, regardless of whether it is declining from a higher value. Ammonia 152 (down from 487) is STILL >150 and STILL triggers this check. Do NOT rationalize that declining ammonia is "improving" when it remains above the lethal threshold. Record: "Pre-Check B triggered: [values]. Predict death."
...
- FIRST: Check Pre-Check B result. If Day 1-3 AND ammonia >150 or lactate >4 not declining or bilirubin rising: predict DEATH immediately. This is BINDING -- do NOT proceed to recovery assessment. A declining ammonia still >150 is STILL lethal.
- SECOND: Check Pre-Check A result. If HE was determined to be uremic, the HE death override is NULLIFIED. Do NOT let grade 4 HE drive a death prediction when the coma is uremic.
...
9. **Recovery Concordance Assessment:** Assess whether organ system improvement is CONCORDANT or DISCORDANT:
   ...
   - **EARLY PRESENTATION WITHOUT TRAJECTORY (day 1-2):** When only 1-2 days of data are available, concordant recovery CANNOT be demonstrated because there is no trajectory. Do NOT assume recovery will occur based on ALFSG-PI alone. A high-severity day-1 presentation with deep HE (grade 3+) + mechanical ventilation + severe AKI (creatinine >= 3.4 mg/dL) is a near-KCC phenotype that the ALFSG-PI fundamentally underestimates (it excludes creatinine and ventilation). Without demonstrated recovery, predict death when this severity pattern is present.
   - **EARLY CRITICAL METABOLIC WARNING (Day 1-3, BINDING):** At Day 1-3, patients may show PARTIAL improvement (e.g., INR declining, creatinine improving) while retaining critical metabolic derangements that predict death. This check evaluates CURRENT VALUES, NOT trends. When ANY of the following are present at Day 1-3, predict DEATH regardless of other improvements: (a) ammonia >150 umol/L -- this is BINDING: ammonia >150 carries significant ICH risk REGARDLESS of whether it is declining from a higher value. An ammonia of 152 declining from 487 is STILL >150 and STILL carries ICH risk. The absolute value determines danger, not the direction. Do NOT rationalize that declining ammonia is "improving" when it remains above the critical threshold. (b) lactate >4 mmol/L AND not declining, (c) bilirubin actively rising. The combination of ammonia >150 + stagnant/rising lactate + rising bilirubin at Day 3 indicates ongoing hepatic necrosis despite partial coagulation improvement. This is BINDING and FINAL: do NOT override with partial improvement in INR or creatinine. This is distinct from Day 5+ demonstrated recovery where multi-day concordant improvement IS conclusive.
...
- **MANDATORY PRE-CHECK B -- EARLY CRITICAL METABOLIC WARNING (Day 1-3, BINDING):** At Day 1-3, check CURRENT VALUES (not trends): ammonia >150 AND/OR lactate >4 not declining AND/OR bilirubin rising = predict DEATH. This is BINDING and FINAL. CRITICAL: ammonia >150 means the CURRENT reading exceeds 150, regardless of decline from a higher value. Ammonia 152 (down from 487) STILL triggers this check. Do NOT rationalize declining ammonia as "improving" when it remains above the lethal threshold. Record: "Pre-Check B triggered: [values]. Predict death."
...
- FIRST: Apply Pre-Check B result. If Day 1-3 AND ammonia >150 or lactate >4 not declining or bilirubin rising: predict DEATH immediately. This is BINDING -- do NOT proceed to recovery assessment. A declining ammonia still >150 is STILL lethal.
- SECOND: Apply Pre-Check A result. If HE is uremic, the HE death override is NULLIFIED. Do NOT let uremic grade 4 HE drive a death prediction.
...
- Only apply death overrides when: (a) Pre-Check B not triggered, AND (b) no recovery criteria met, AND (c) HE is not uremic.
...
8. **Recovery Concordance Framework:** In APAP/hyperacute ALF, true recovery shows CONCORDANT improvement: INR normalizing toward <2.0, HE resolving toward grade 0-1, lactate normalizing, creatinine improving, ALT declining. When this concordant pattern is present, bilirubin may continue to rise for days (lagging indicator, typically ~8-12 mg/dL) and isolated vasopressor use with normal lactate should not override the recovery signal. Conversely, DISCORDANT patterns indicate failed regeneration.
   **OVERRIDE CONDITIONS (context-dependent):**
   ...
   - **EARLY CRITICAL METABOLIC WARNING (Day 1-3, BINDING):** At Day 1-3, patients may show PARTIAL improvement while retaining critical metabolic derangements. This check evaluates CURRENT VALUES, NOT trends. When ANY of the following are present at Day 1-3, predict DEATH regardless of other improvements: (a) ammonia >150 umol/L -- this is BINDING: ammonia >150 carries significant ICH risk REGARDLESS of whether it is declining from a higher value. An ammonia of 152 declining from 487 is STILL >150 and STILL carries ICH risk. The absolute value determines danger, not the direction. Do NOT rationalize declining ammonia as "improving" when it remains above the critical threshold. (b) lactate >4 mmol/L AND not declining, (c) bilirubin actively rising. Partial improvement at Day 3 does NOT establish recovery. >30% of "too sick" patients survive (Dong 2024), but this applies to patients with TRAJECTORY -- not to Day 3 patients with persistent critical metabolic failure. This is BINDING and FINAL.
...
- **MANDATORY PRE-CHECK B -- EARLY METABOLIC WARNING (Day 1-3, BINDING):** Check CURRENT VALUES (not trends): ammonia >150 AND/OR lactate >4 not declining AND/OR bilirubin rising = predict DEATH. This is BINDING and FINAL. CRITICAL: ammonia >150 means the CURRENT reading exceeds 150, regardless of decline from a higher value. Ammonia 152 (down from 487) STILL triggers this check. Do NOT rationalize declining ammonia as "improving" when it remains above the lethal threshold. Record: "Pre-Check B triggered: [values]. Predict death."
...
**MANDATORY PRE-CHECK B -- EARLY METABOLIC WARNING (Day 1-3, BINDING):**
At Day 1-3, check CURRENT VALUES (not trends): ammonia >150 AND/OR lactate >4 not declining AND/OR bilirubin rising = predict DEATH. This is BINDING and FINAL. CRITICAL: ammonia >150 means the CURRENT reading exceeds 150, regardless of decline from a higher value. Ammonia 152 (down from 487) is STILL >150 and STILL triggers this check -- the absolute value determines danger, not the direction of change. Do NOT let ANY specialist rationalize that declining ammonia is "improving" when it remains above the lethal threshold. If ANY specialist's data shows ammonia >150 at Day 1-3, predict death regardless of other improvements.
...
1. **Pre-Check B (BINDING):** If Day 1-3 AND ammonia >150 or lactate >4 not declining or bilirubin rising: predict DEATH. This takes PRIORITY over any partial improvement or recovery signal. Do NOT let partial INR or creatinine improvement override this. A declining ammonia that is still >150 is STILL dangerous.
...
3. If ANY recovery criteria met (Day 5+) AND no multi-organ active deterioration: predict SURVIVAL regardless of weighted vote.
4. If Hepatologist identifies liver recovery AND predicts survival: give DECISIVE weight. Remaining organ failures with a recovered liver are ICU-manageable.
5. COMBINATION: moderate-INR recovery + uremic HE = predict SURVIVAL.
6. If non-uremic death override met WITHOUT recovery criteria: predict death.
