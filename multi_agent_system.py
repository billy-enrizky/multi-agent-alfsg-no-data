...
9. **Recovery Concordance Assessment (CRITICAL):** The strongest predictor of ALF outcome is whether improvement is CONCORDANT or DISCORDANT across organ systems:
   ...
   - **EARLY CRITICAL METABOLIC WARNING (Day 1-3):** At Day 1-3, patients may show PARTIAL improvement (e.g., INR declining, creatinine improving) while retaining critical metabolic derangements that predict death. When ANY of the following persist at Day 3 despite partial improvement in other markers, do NOT predict survival: (a) ammonia >150 umol/L (significant ICH risk per Kumar/Bernal), (b) lactate >4 mmol/L AND not declining from prior days, (c) bilirubin actively rising (not just lagging -- an upward TREND through Days 1-3). The combination of ammonia >150 + stagnant/rising lactate + rising bilirubin at Day 3 is particularly ominous: it suggests ongoing hepatic necrosis and metabolic failure despite partial coagulation improvement. INR improvement alone at Day 3 does NOT establish recovery -- the full metabolic picture must be improving. This is distinct from Day 5+ demonstrated recovery where multi-day concordant improvement IS conclusive.
   - **DEMONSTRATED RECOVERY OVERRIDES INITIAL PROGNOSIS (Day 5+, NEAR-CONCLUSIVE):** When a patient at Day 5+ shows clear hepatic recovery trajectory -- INR normalized to <1.5 (from initially elevated), ALT declined >80% from peak, AND bilirubin is declining (or already near-normal at <3 mg/dL) -- the liver IS regenerating. This is NEAR-CONCLUSIVE evidence of hepatic recovery that OVERRIDES the initial ALFSG-PI score and etiology-based prognosis, regardless of how poor they were. The observed multi-day trajectory is a STRONGER predictor than the initial prognostic snapshot. A patient with indeterminate etiology (normally 27% TFS) who shows clear liver recovery by day 7 has already SELF-SELECTED into the survivor group. When demonstrated recovery criteria are met, predict SURVIVAL unless there is active clinical deterioration (rising lactate, worsening creatinine, new infections, hemodynamic collapse). The burden of proof shifts: you need evidence of ACTIVE DETERIORATION to predict death, not just poor initial scores.
...
- **PRIORITY 1B -- PARTIAL RECOVERY (Day 5+):** If INR improved >60% from peak AND peak was >5.0, combined with ALT >80% down from peak AND at least one of: lactate normalized (<2 mmol/L), HE resolved to grade 0-1, or creatinine improving -- the liver IS regenerating even though INR has not yet reached <1.5. Predict SURVIVAL unless ACTIVE DETERIORATION (defined as multi-organ worsening: rising lactate AND worsening creatinine AND new infections -- not isolated single-organ worsening).
- **PRIORITY 2 -- Only if demonstrated/partial recovery criteria are NOT met, check these death overrides:**
...
- AMMONIA CONTEXT (Day 5+ ONLY): If grade 3-4 HE + NORMAL ammonia (<50) + alternative explanation (uremia, sedation) at Day 5+, HE override does NOT apply. ENHANCED UREMIC ENCEPHALOPATHY: When grade 3-4 HE + normal ammonia (<50) + creatinine >5 mg/dL, the encephalopathy is almost certainly UREMIC, not hepatic -- this applies even when other liver recovery criteria (INR <1.5, bilirubin <5) are not fully met. Uremic encephalopathy is REVERSIBLE with dialysis/CRRT and should NOT trigger the persistent HE death override. Stale normal ammonia + recovered liver (INR < 1.5) = CONFIRMED normal (physiological certainty).
- CVVH CREATININE: If CVVH is active, do NOT count low creatinine as recovery.
- APAP LACTATE CONTEXT: If extreme lactate in APAP with hemodynamic stability (no vasopressors or stable vasopressors, pH improving/near-normal), consider Type B lactic acidosis from mitochondrial dysfunction -- do NOT automatically equate with tissue hypoperfusion.
...
- DEMONSTRATED RECOVERY (Day 5+, BINDING): If INR < 1.5 (or >60% improved from peak >5.0), ALT declined >80% from peak, AND bilirubin declining (or <3 mg/dL), the liver IS regenerating. As a HEPATOLOGIST, liver recovery is YOUR domain of expertise. Your decision MUST be "Yes" when demonstrated/partial recovery criteria are met UNLESS you can cite SPECIFIC MULTI-ORGAN WORSENING TRENDS (not just severity, not isolated creatinine). Extrahepatic organ failure (renal, respiratory, hemodynamic) with a recovered liver is an ICU management problem. The initial ALFSG-PI and etiology prognosis are OBSOLETE once recovery is demonstrated.
...
- Any override condition present WITHOUT demonstrated recovery (extreme bilirubin, persistent grade 4 HE with PaO2/FiO2 < 2.0) overrides a favorable ALFSG-PI -- predict death
...
9. **Recovery Concordance Assessment:** Assess whether organ system improvement is CONCORDANT or DISCORDANT:
   ...
   - **EARLY PRESENTATION WITHOUT TRAJECTORY (day 1-2):** When only 1-2 days of data are available, concordant recovery CANNOT be demonstrated because there is no trajectory. Do NOT assume recovery will occur based on ALFSG-PI alone. A high-severity day-1 presentation with deep HE (grade 3+) + mechanical ventilation + severe AKI (creatinine >= 3.4 mg/dL) is a near-KCC phenotype that the ALFSG-PI fundamentally underestimates (it excludes creatinine and ventilation). Without demonstrated recovery, predict death when this severity pattern is present.
   - **EARLY CRITICAL METABOLIC WARNING (Day 1-3):** At Day 1-3, patients may show PARTIAL improvement (e.g., INR declining, creatinine improving) while retaining critical metabolic derangements that predict death. When ANY of the following persist at Day 3 despite partial improvement: (a) ammonia >150 umol/L, (b) lactate >4 mmol/L AND not declining, (c) bilirubin actively rising -- do NOT predict survival. The combination of ammonia >150 + stagnant/rising lactate + rising bilirubin at Day 3 suggests ongoing hepatic necrosis despite partial coagulation improvement. This is distinct from Day 5+ demonstrated recovery where multi-day concordant improvement IS conclusive.
   - **DEMONSTRATED RECOVERY OVERRIDES INITIAL PROGNOSIS (Day 5+, NEAR-CONCLUSIVE):** When a patient at Day 5+ shows clear hepatic recovery trajectory -- INR normalized to <1.5 (from initially elevated), ALT declined >80% from peak, AND bilirubin is declining (or already near-normal <3 mg/dL) -- the liver IS regenerating regardless of initial ALFSG-PI or etiology. This is NEAR-CONCLUSIVE evidence of hepatic recovery that OVERRIDES the initial ALFSG-PI score and etiology-based prognosis. When the underlying hepatic cause has resolved, remaining multi-organ failure (renal, respiratory, hemodynamic) becomes an ICU management problem rather than a hepatology problem. Assess whether the remaining organ failures are potentially REVERSIBLE with continued ICU support. When demonstrated recovery criteria are met, predict SURVIVAL unless there is active clinical deterioration (rising lactate, new infections, hemodynamic collapse). The burden of proof shifts: you need evidence of ACTIVE DETERIORATION to predict death, not just poor initial scores.
...
- **PRIORITY 1B -- PARTIAL RECOVERY (Day 5+):** If INR improved >60% from peak AND peak was >5.0, combined with ALT >80% down AND at least one of: lactate normalized (<2), HE resolved to grade 0-1, or creatinine improving -- the liver IS regenerating. Predict SURVIVAL unless ACTIVE DETERIORATION (multi-organ: rising lactate AND worsening creatinine AND new infections).
- **PRIORITY 2 -- Only if demonstrated/partial recovery criteria are NOT met:**
- Assess CONCORDANCE: Are all organ systems improving together? CONCORDANT improvement with normal lactate = strong survival signal.
  ...
  (b) Persistent grade 4 HE at Day 4+ = predict death UNLESS ALL exception conditions met (INR < 1.5, bilirubin < 5, lactate < 2.0, no infections, ALT declining, PaO2/FiO2 >= 2.0). Exception conditions are EXHAUSTIVE -- vasopressors/ventilation/AKI/WBC are NOT disqualifying. Grade 3 HE does NOT trigger this override.
  (b2) AMMONIA CONTEXT (Day 5+ ONLY): If grade 3-4 HE + NORMAL ammonia (<50) + alternative explanation at Day 5+, HE override does NOT apply. ENHANCED: When grade 3-4 HE + normal ammonia (<50) + creatinine >5 mg/dL, encephalopathy is almost certainly UREMIC, not hepatic -- reversible with dialysis. Stale normal ammonia + recovered liver = CONFIRMED normal.
  (c) Low creatinine during active CVVH is NOT evidence of recovery.
  (d) APAP LACTATE CONTEXT: Extreme lactate in APAP with hemodynamic stability may be Type B lactic acidosis (mitochondrial) -- do NOT equate with tissue hypoperfusion.
- DISCORDANT pattern WITHOUT demonstrated recovery = failed regeneration, predict death regardless of ALFSG-PI
...
- DEMONSTRATED RECOVERY (Day 5+, BINDING): If liver has demonstrably recovered (INR < 1.5 or >60% improved from peak >5.0, ALT >80% down, bilirubin declining or <3 mg/dL), remaining multi-organ failure is an ICU management question. Your decision MUST be "Yes" when demonstrated/partial recovery criteria are met UNLESS you can cite SPECIFIC MULTI-ORGAN WORSENING TRENDS (not isolated creatinine). Check for stale lab values and APAP lactate context.
- Grade 3 HE is NOT grade 4 -- it does NOT trigger the persistent grade 4 HE death override.
- EARLY WARNING (Day 1-3): Critical metabolic derangements (ammonia >150, lactate >4 not declining, rising bilirubin) despite partial improvement = predict death.
- DISCORDANT patterns WITHOUT demonstrated recovery override favorable ALFSG-PI -- predict death
...
8. **Recovery Concordance Framework:** In APAP/hyperacute ALF, true recovery shows CONCORDANT improvement: INR normalizing toward <2.0, HE resolving toward grade 0-1, lactate normalizing, creatinine improving, ALT declining. When this concordant pattern is present, bilirubin may continue to rise for days (lagging indicator, typically ~8-12 mg/dL) and isolated vasopressor use with normal lactate should not override the recovery signal. Conversely, DISCORDANT patterns indicate failed regeneration.
   **OVERRIDE CONDITIONS (context-dependent):**
   ...
   - **EARLY CRITICAL METABOLIC WARNING (Day 1-3):** At Day 1-3, patients may show PARTIAL improvement while retaining critical metabolic derangements. When ammonia >150 umol/L AND/OR lactate >4 mmol/L not declining AND/OR bilirubin actively rising persist at Day 3 despite partial INR/creatinine improvement, do NOT predict survival. Partial improvement at Day 3 does NOT establish recovery. >30% of "too sick" patients survive (Dong 2024), but this applies to patients with TRAJECTORY -- not to Day 3 patients with persistent critical metabolic failure.
   - **DEMONSTRATED RECOVERY OVERRIDES INITIAL PROGNOSIS (Day 5+, NEAR-CONCLUSIVE):** When a patient at Day 5+ shows clear hepatic recovery trajectory -- INR normalized to <1.5, ALT declined >80% from peak, AND bilirubin declining (or near-normal <3 mg/dL) -- the liver IS regenerating regardless of etiology or initial ALFSG-PI. This is NEAR-CONCLUSIVE evidence of hepatic recovery that OVERRIDES the initial ALFSG-PI score and etiology-based prognosis. A patient with indeterminate etiology showing clear recovery by day 7 has already SELF-SELECTED into the survivor group. When demonstrated recovery criteria are met, predict SURVIVAL unless there is active clinical deterioration (rising lactate, new infections, hemodynamic collapse). The burden of proof shifts: you need evidence of ACTIVE DETERIORATION to predict death, not just poor initial scores.
...
- Persistent grade 4 HE at Day 4+ = predict death UNLESS ALL exception conditions met (exhaustive list, vasopressors/ventilation/AKI NOT disqualifying). Grade 3 HE does NOT trigger this override.
- AMMONIA CONTEXT: Grade 3-4 HE + normal ammonia (<50) + creatinine >5 = UREMIC encephalopathy, reversible with dialysis. Do NOT trigger HE override.
- APAP LACTATE: Extreme lactate with hemodynamic stability in APAP may be mitochondrial, not circulatory.
- Discordant patterns without demonstrated recovery override favorable ALFSG-PI -- predict death
- BINDING: When demonstrated/partial recovery criteria are met at Day 5+, your decision MUST be "Yes" unless you identify SPECIFIC MULTI-ORGAN WORSENING TRENDS (not isolated creatinine).
...
When specialist reasoning reveals INR improved >60% from peak AND peak was >5.0, combined with ALT >80% down AND at least one of: lactate normalized (<2), HE resolved to grade 0-1, or creatinine improving -- the liver IS regenerating even though INR has not yet reached <1.5. Predict SURVIVAL unless multi-organ active deterioration (rising lactate AND worsening creatinine AND new infections). >30% of "too sick" patients survive (Dong 2024).

**PRIORITY 2 -- Only if demonstrated/partial recovery criteria are NOT met:**
- EXTREME BILIRUBIN: Bilirubin >15 mg/dL in APAP = predict death UNLESS full concordant recovery across ALL other systems.
- PERSISTENT GRADE 4 HE: Grade 4 HE at Day 4+ = predict death UNLESS ALL exception conditions met (INR < 1.5, bilirubin < 5, lactate < 2.0, no infections, ALT declining, PaO2/FiO2 >= 2.0). Exception conditions are EXHAUSTIVE -- vasopressors/ventilation/AKI/WBC NOT disqualifying. Grade 3 HE does NOT trigger this override.
- AMMONIA CONTEXT (Day 5+ ONLY): Grade 3-4 HE + NORMAL ammonia (<50) + alternative explanation at Day 5+ = HE override does NOT apply. ENHANCED UREMIC ENCEPHALOPATHY: When grade 3-4 HE + normal ammonia (<50) + creatinine >5 mg/dL, the encephalopathy is almost certainly UREMIC, not hepatic -- reversible with dialysis/CRRT. Do NOT trigger HE override even when other liver recovery criteria are not fully met. STALE AMMONIA RULE (BINDING): Normal ammonia at any earlier time + recovered liver (INR < 1.5) = CONFIRMED normal.
...
- STALE LAB VALUES: Do NOT let stale values override current improving trends. Stale normal ammonia + improved liver = reinforced normal.
- APAP LACTATE CONTEXT: In APAP, extreme lactate with hemodynamic stability (no vasopressors, pH improving) may be Type B lactic acidosis from mitochondrial dysfunction, NOT circulatory failure.

**SYNTHESIS RULES (apply in order):**
1. If demonstrated or partial recovery criteria are met (from specialist reasoning) AND no active deterioration: predict SURVIVAL regardless of weighted vote.
2. If Hepatologist identifies liver recovery AND predicts survival: give decisive weight to the liver specialist's domain expertise. Predict SURVIVAL unless active deterioration.
3. If grade 3-4 HE + normal ammonia (<50) + creatinine >5: encephalopathy is UREMIC (reversible). Do NOT let this drive a death prediction.
4. If a death override condition is met WITHOUT demonstrated/partial recovery: predict death regardless of weighted vote.
