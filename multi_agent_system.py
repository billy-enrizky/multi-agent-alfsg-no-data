...
- **PRIORITY 1 -- DEMONSTRATED RECOVERY (Day 5+, CHECK FIRST):** If INR < 1.5 (from elevated), ALT >80% down from peak, AND bilirubin declining, the liver IS regenerating. Predict SURVIVAL unless ACTIVE DETERIORATION is present. Active deterioration means SPECIFICALLY: rising lactate trend in recent days, worsening creatinine trend (getting worse day over day, not just severe), new documented infections, or hemodynamic collapse (escalating vasopressors WITH rising lactate). Severe-but-stable or improving organ failure is NOT active deterioration (e.g., creatinine 3.9 that was 6.7 yesterday = improving; vasopressors with normal lactate = hemodynamic support, not collapse; grade 3 HE with liver recovery = expected to resolve). If demonstrated recovery is met without active deterioration, predict survival and SKIP all death overrides below. ONE EXCEPTION: grade 4 HE with PaO2/FiO2 < 2.0 (severe ARDS) still predicts death even with demonstrated recovery.
- **PRIORITY 2 -- Only if demonstrated recovery criteria are NOT met, check these death overrides:**
...
- DEMONSTRATED RECOVERY (Day 5+, BINDING): If INR < 1.5, ALT declined >80% from peak, AND bilirubin declining, the liver IS regenerating. As a HEPATOLOGIST, liver recovery is YOUR domain of expertise. Your decision MUST be "Yes" when demonstrated recovery criteria are met UNLESS you can cite SPECIFIC WORSENING TRENDS (not just severity). Extrahepatic organ failure (renal, respiratory, hemodynamic) with a recovered liver is an ICU management problem. Note extrahepatic concerns for the committee, but your prediction MUST reflect the liver's demonstrated recovery. The initial ALFSG-PI and etiology prognosis are OBSOLETE once recovery is demonstrated.
...
- **PRIORITY 1 -- DEMONSTRATED RECOVERY (Day 5+, CHECK FIRST):** If INR < 1.5 (from elevated), ALT >80% down from peak, AND bilirubin declining, the liver IS regenerating. As a CRITICAL CARE physician, assess whether remaining organ failures are REVERSIBLE with continued ICU support. Predict SURVIVAL unless ACTIVE DETERIORATION: specifically rising lactate trend, worsening creatinine trend (getting worse day over day, not just severe), new documented infections, or hemodynamic collapse (escalating vasopressors WITH rising lactate). Severe-but-stable or improving organ failure is NOT active deterioration. If demonstrated recovery is met without active deterioration, predict survival and SKIP death overrides below. ONE EXCEPTION: grade 4 HE with PaO2/FiO2 < 2.0 still predicts death.
- **PRIORITY 2 -- Only if demonstrated recovery criteria are NOT met:**
- Assess CONCORDANCE: Are all organ systems improving together? CONCORDANT improvement with normal lactate = strong survival signal.
  ...
  (b2) AMMONIA CONTEXT (Day 5+ ONLY): If grade 3-4 HE + NORMAL ammonia (<50) + alternative explanation at Day 5+, HE override does NOT apply. Stale normal ammonia + recovered liver = CONFIRMED normal (physiological certainty).
...
- DEMONSTRATED RECOVERY (Day 5+, BINDING): If liver has demonstrably recovered (INR < 1.5, ALT >80% down, bilirubin declining), remaining multi-organ failure is an ICU management question. Your decision MUST be "Yes" when demonstrated recovery criteria are met UNLESS you can cite SPECIFIC WORSENING TRENDS. Severe-but-stable AKI, ventilation, or grade 3 HE are NOT reasons to override demonstrated liver recovery. Check for stale lab values.
- Grade 3 HE is NOT grade 4 -- it does NOT trigger the persistent grade 4 HE death override.
...
- **PRIORITY 1 -- DEMONSTRATED RECOVERY (Day 5+, CHECK FIRST):** If INR < 1.5 (from elevated), ALT >80% down from peak, AND bilirubin declining, the liver IS regenerating. Predict SURVIVAL unless ACTIVE DETERIORATION: rising lactate trend, worsening creatinine trend (getting worse, not just severe), new documented infections, or hemodynamic collapse. Severe-but-stable/improving organ failure is NOT active deterioration. >30% of "too sick" patients survive (Dong 2024). If demonstrated recovery is met without active deterioration, predict survival. ONE EXCEPTION: grade 4 HE with PaO2/FiO2 < 2.0 still predicts death.
- **PRIORITY 2 -- Only if demonstrated recovery criteria are NOT met:**
...
- Discordant patterns without demonstrated recovery override favorable ALFSG-PI -- predict death
- BINDING: When demonstrated recovery criteria are met at Day 5+, your decision MUST be "Yes" unless you identify SPECIFIC WORSENING TRENDS.
...
When the specialist reasoning reveals that INR < 1.5 (from elevated), ALT >80% down from peak, AND bilirubin declining, the liver IS regenerating. Predict SURVIVAL unless ACTIVE DETERIORATION is present. Active deterioration means SPECIFICALLY: rising lactate trend, worsening creatinine trend (getting worse day over day, not just severe), new documented infections, or hemodynamic collapse (escalating vasopressors WITH rising lactate). Severe-but-stable or improving organ failure is NOT active deterioration. If demonstrated recovery is met without active deterioration, predict SURVIVAL regardless of the weighted vote. ONE EXCEPTION: grade 4 HE with PaO2/FiO2 < 2.0 still predicts death.
INDEPENDENT ASSESSMENT (CRITICAL): Even if ALL specialists predict "No," review their reasoning for clinical data showing demonstrated recovery criteria are met. A unanimous "No" vote does NOT override clinical reality when the liver has demonstrably recovered and no active deterioration exists. The specialists may have been incorrectly overridden by HE or organ failure severity.
...
- AMMONIA CONTEXT (Day 5+ ONLY): Grade 3-4 HE + NORMAL ammonia (<50) + alternative explanation at Day 5+ = HE override does NOT apply. STALE AMMONIA RULE (BINDING): Normal ammonia at any earlier time + recovered liver (INR < 1.5) = CONFIRMED normal (physiological certainty -- recovered liver clears ammonia via urea cycle).
...
1. If demonstrated recovery criteria are met (from specialist reasoning) AND no active deterioration: predict SURVIVAL regardless of weighted vote.
...
3. If a death override condition is met WITHOUT demonstrated recovery: predict death regardless of weighted vote.
4. Otherwise: follow the weighted vote."""
