...
9. **Recovery Concordance Assessment (CRITICAL):** The strongest predictor of ALF outcome is whether improvement is CONCORDANT or DISCORDANT across organ systems:
   ...
   - **PERSISTENT GRADE 4 HE (DURATION-BASED):** APAP recovery typically shows rapid neurological clearing within 2-3 days. Persistent grade 4 HE at Day 4+ in APAP (or Day 5+ in non-APAP) is a critical death signal because 28% of ALF deaths are neurologic (cerebral edema/herniation, Karvellas 2023). The ALFSG-PI treats grades 3 and 4 identically (both "deep"), but grade 4 carries substantially higher mortality risk. Predict death when grade 4 HE persists at Day 4+ UNLESS liver function has FULLY normalized (INR < 1.5, bilirubin < 5 mg/dL, lactate < 2.0 mmol/L, no active infections, ALT declining). Rising lactate strengthens this override but is NOT required -- persistent grade 4 HE alone at Day 4+ with any other concerning feature (elevated lactate, bilirubin > 5, infection, high INR) is sufficient to predict death.
   - **AMMONIA CONTEXT FOR HE ASSESSMENT:** Hepatic encephalopathy is driven by hyperammonemia. When persistent grade 3-4 HE coexists with NORMAL ammonia (<50 umol/L), the altered consciousness is likely NON-hepatic in origin (sedation for mechanical ventilation, uremic encephalopathy from AKI, ICU delirium). In this context, the deep HE does NOT carry the same cerebral edema/herniation risk and should NOT trigger the persistent HE death override. Instead, assess whether the non-hepatic causes of coma are potentially reversible (e.g., AKI improving with creatinine declining, sedation can be weaned when respiratory status improves).
   ...
   - **DEMONSTRATED RECOVERY OVERRIDES INITIAL PROGNOSIS (Day 5+):** When a patient at Day 5+ shows clear hepatic recovery trajectory -- INR normalized to <1.5 (from initially elevated), ALT declined >80% from peak, AND bilirubin is declining -- the liver IS regenerating regardless of etiology or initial ALFSG-PI. The observed multi-day trajectory is a stronger predictor than the initial prognostic score. A patient with indeterminate etiology (normally 27% TFS) who shows clear liver recovery by day 7 is fundamentally different from the average indeterminate patient at presentation. DOWNWEIGHT the initial ALFSG-PI and etiology prognosis in favor of the demonstrated trajectory.
   - **STALE LAB VALUE CAUTION:** Some lab values may be from earlier days (indicated by "from day X"). Values more than 2 days old should be interpreted with significant caution, particularly lactate and ammonia which change rapidly. In a patient showing clear recovery trajectory at day 5+, a stale elevated lactate from early presentation should NOT override current improving trends.
...
- AMMONIA CONTEXT: If persistent grade 3-4 HE coexists with NORMAL ammonia (<50 umol/L), the coma is likely non-hepatic (sedation, uremia, ICU delirium). Do NOT apply the persistent HE death override -- assess reversibility of non-hepatic causes instead.
...
9. **Recovery Concordance Assessment:** Assess whether organ system improvement is CONCORDANT or DISCORDANT:
   ...
   - **PERSISTENT GRADE 4 HE (duration-based):** APAP recovery typically clears HE within 2-3 days. Grade 4 HE persisting at Day 4+ in APAP (or Day 5+ in non-APAP) = predict death UNLESS liver function has FULLY normalized (INR < 1.5, bilirubin < 5 mg/dL, lactate < 2.0, no infections, ALT declining). Rising lactate strengthens this but is NOT required -- persistent grade 4 HE at Day 4+ with ANY other concerning feature (elevated lactate, bilirubin > 5, infection, elevated INR) is sufficient to predict death. ALFSG-PI does not distinguish grade 3 from grade 4.
   - **AMMONIA CONTEXT FOR HE ASSESSMENT:** Hepatic encephalopathy is driven by hyperammonemia. When persistent grade 3-4 HE coexists with NORMAL ammonia (<50 umol/L), the altered consciousness is likely NON-hepatic in origin (sedation for mechanical ventilation, uremic encephalopathy from AKI, ICU delirium). In this context, the deep HE does NOT carry the same cerebral edema/herniation risk and should NOT trigger the persistent HE death override. Instead, assess whether the non-hepatic causes of coma are potentially reversible (e.g., AKI improving with creatinine declining, sedation can be weaned when respiratory status improves).
   ...
   - **EARLY PRESENTATION WITHOUT TRAJECTORY (day 1-2):** When only 1-2 days of data are available, concordant recovery CANNOT be demonstrated because there is no trajectory. Do NOT assume recovery will occur based on ALFSG-PI alone. A high-severity day-1 presentation with deep HE (grade 3+) + mechanical ventilation + severe AKI (creatinine >= 3.4 mg/dL) is a near-KCC phenotype that the ALFSG-PI fundamentally underestimates (it excludes creatinine and ventilation). Without demonstrated recovery, predict death when this severity pattern is present.
   - **DEMONSTRATED RECOVERY OVERRIDES INITIAL PROGNOSIS (Day 5+):** When a patient at Day 5+ shows clear hepatic recovery trajectory -- INR normalized to <1.5 (from initially elevated), ALT declined >80% from peak, AND bilirubin is declining -- the liver IS regenerating regardless of initial ALFSG-PI or etiology. When the underlying hepatic cause has resolved, remaining multi-organ failure (renal, respiratory, hemodynamic) becomes an ICU management problem rather than a hepatology problem. Assess whether the remaining organ failures are potentially REVERSIBLE with continued ICU support (creatinine improving, respiratory support manageable, hemodynamics stabilizing with vasopressors and normal lactate). DOWNWEIGHT the initial ALFSG-PI in favor of the demonstrated recovery trajectory.
   - **STALE LAB VALUE CAUTION:** Some lab values may be from earlier days (indicated by "from day X"). Values more than 2 days old should be interpreted with significant caution, particularly lactate and ammonia which change rapidly. In a patient showing clear recovery at day 5+, a stale elevated lactate from early presentation should NOT override current improving trends.
...
- AMMONIA CONTEXT: If grade 3-4 HE coexists with NORMAL ammonia (<50 umol/L), the altered consciousness is likely non-hepatic (sedation, uremia). Do NOT apply the persistent HE death override in this context.
...
- Check OVERRIDE CONDITIONS before concluding concordant recovery:
  ...
  (b) Persistent grade 4 HE at Day 4+ in APAP (or Day 5+ in non-APAP) = predict death UNLESS liver function fully normalized (INR < 1.5, bilirubin < 5, lactate < 2.0, no infections). Rising lactate strengthens but is NOT required -- persistent grade 4 HE + any other concerning feature is sufficient.
  (b2) AMMONIA CONTEXT: If persistent grade 3-4 HE has NORMAL ammonia (<50 umol/L), the coma is likely non-hepatic. Do NOT apply the persistent HE death override -- assess reversibility instead.
...
- DEMONSTRATED RECOVERY (Day 5+): If the liver has demonstrably recovered (INR < 1.5, ALT >80% down, bilirubin declining), remaining multi-organ failure is an ICU management question, not a hepatology question. Assess reversibility of remaining failures rather than predicting death from multi-organ failure alone. Check for stale lab values.
...
8. **Recovery Concordance Framework:** In APAP/hyperacute ALF, true recovery shows CONCORDANT improvement: INR normalizing toward <2.0, HE resolving toward grade 0-1, lactate normalizing, creatinine improving, ALT declining. When this concordant pattern is present, bilirubin may continue to rise for days (lagging indicator, typically ~8-12 mg/dL) and isolated vasopressor use with normal lactate should not override the recovery signal. Conversely, DISCORDANT patterns indicate failed regeneration.
   **OVERRIDE CONDITIONS (context-dependent):**
   ...
   - PERSISTENT GRADE 4 HE (duration-based): APAP recovery clears HE within 2-3 days. Grade 4 HE persisting at Day 4+ in APAP (or Day 5+ in non-APAP) = predict death UNLESS liver function fully normalized (INR < 1.5, bilirubin < 5, lactate < 2.0, no infections, ALT declining). Rising lactate strengthens but is NOT required -- persistent grade 4 HE at Day 4+ with ANY other concerning feature is sufficient. ALFSG-PI treats grade 3 and 4 identically but grade 4 has much higher mortality.
   - AMMONIA CONTEXT FOR HE ASSESSMENT: Hepatic encephalopathy is driven by hyperammonemia. When persistent grade 3-4 HE coexists with NORMAL ammonia (<50 umol/L), the altered consciousness is likely NON-hepatic in origin (sedation, uremic encephalopathy, ICU delirium). The deep HE does NOT carry the same cerebral edema/herniation risk and should NOT trigger the persistent HE death override. Assess reversibility of non-hepatic causes instead.
